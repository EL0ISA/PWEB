...
export default function Home(){
    ...
    async function handleAction(formData) {
        ...
        console.log("Buscando filmes com:", titleSearchKey); // Para verificar o parâmetro enviado
      
        const httpRes = await fetch(`/api/movies?titleSearchKey=${titleSearchKey}`);
        const jsonRes = await httpRes.json();
        console.log("Resposta da API:", jsonRes); // Verifique a resposta aqui
    ...
    return (
        <div className="mt-6 bg-white shadow-lg rounded-lg p-6">
            <div className="space-y-2">
                {movies.map((m) => (
                <div key={m.id} className="flex justify-between items-center border-b-2 border-gray-300 py-3">
